...
| | |
|---|---|
| Parameters | 489.3M (448.3M excluding embeddings) |
| Layers and width | 24 layers, d_model 1280 |
| Attention | Grouped-query, 20 query heads sharing 4 key/value heads |
| Feed-forward | SwiGLU, d_ff 3840 |
| Normalization and positions | Pre-norm RMSNorm, RoPE |
| Vocabulary | 32,000-token byte-level BPE, trained on the same corpus |
| Context | 2048 tokens |
| Batch | 524,288 tokens per step |
...
<figcaption>Validation loss from 0.5B tokens on. Phase 3 is still running; the dashed segment is its first 100 steps. Hover a point to see its exact value.</figcaption>
</figure>

Raising the learning rate on a model that had just been annealed undid part of that annealing. Validation loss rose from 3.323 to 3.383 and took about 590 steps to get back, roughly 15% of phase 2. Phase 3 shows the same jump: 3.139 to 3.183 in its first 100 steps.
